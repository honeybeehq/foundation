...
All publication here uses real Comb with a private local backend. This proves local adapter/merge behavior, not remote credentials, two-machine networking, backend outages, or ambiguous remote acknowledgments. Disconnect only pauses publication; local editing still requires the host. Connected alone is insufficient. `restart` restarts a client, not the host; a full host restart/recovery claim needs a separate acceptance path. For a packaged app starting its own host, run the existing `live-proof.mjs` with a fresh proof directory.
